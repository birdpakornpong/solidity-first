...
contract Will {
    ...
    event Tranfer(address indexed owner, address indexed heir, uint256 amount);

    constructor() {
        // เรียกครั้งเดียวตอน deploy
        _admin = msg.sender;
    }
    ...
    modifier isHadWill(address willBuilder) {
        require(
            _will[willBuilder].amount > 0,
            "the person who said it did not had will"
        );
        _;
    }
    ...
    function checkWill(address willBuilder)
        public
        view
        isHadWill(willBuilder)
        returns (address heir, uint256 amount)
    {
        return (_will[willBuilder].heir, _will[willBuilder].amount);
    }

    function tranferForDie(address whoDie)
        public
        payable
        isAdmin
        isHadWill(whoDie)
    ...
    function checkTotal() public view isAdmin returns (uint256 Balance) {
        return _balance;
    }
